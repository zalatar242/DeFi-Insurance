...
describe("InsurancePool", function () {
  ...
  describe("Coverage Purchase", function () {
    it("Should allow purchasing coverage with security deposit", async function () {
      ...
      const utilization = await insurancePool.getUtilizationRate(0); // 0 for STABLECOIN_DEPEG
  ...
  describe("Liquidity Provision", function () {
    it("Should allow adding liquidity", async function () {
      ...
      const bucket = await insurancePool.getRiskBucket(0); // 0 for STABLECOIN_DEPEG
      expect(bucket.allocatedLiquidity).to.equal(liquidityAmount);
    });
  ...
  describe("Premium Calculation", function () {
    it("Should calculate premium based on utilization", async function () {
      ...
      // Add some coverage to increase utilization
      const deposit = coverageAmount.mul(20).div(100);
      await rlusd.connect(buyer).approve(insurancePool.address, deposit);
      await insurancePool.connect(buyer).purchaseCoverage(coverageAmount);

      // Premium should be higher with higher utilization
      const newPremium = await insurancePool.calculatePremium(coverageAmount);
      expect(newPremium).to.be.gt(premium);
    });
  });
